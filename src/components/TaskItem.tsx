...
const TaskItem = memo(({ id, name, completed, onUpdateTask, onDeleteTask }: taskItemProps) => {
    ...
    return (
        <li className='flex grow-0 shrink-0 items-center gap-2 w-full'>
            <input className="hidden grow-0 shrink-0" type="checkbox" id={checkboxId} checked={completed} onChange={() => onUpdateTask(id)} />
            <label className="flex items-center grow px-2 py-1.5 rounded cursor-pointer hover:bg-gray-100 border" htmlFor={checkboxId}>
                ...
                <p className={`ml-2 text-lg grow px-2 rounded-sm break-all ${completed ? 'text-[#9CA3AF] line-through' : 'text-gray-900'}`}>{name}</p>
            </label>
            ...
            <div className="group relative grow-0 shrink-0 flex flex-col align-bottom">
                ...
                <p className="absolute top-8 right-0 scale-0 z-10 rounded bg-gray-800 p-2 text-xs text-white group-hover:scale-100">Delete</p>
            </div>
        </li>
    )
})
